...
const ModernCalendar = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (isEditing && editingEventId) {
      ...
      try {
        const response = await axios.post("http://localhost:8001/api/events/", payload);
        const created = {
          id: response.data.id,
          title: newTitle,
          start: startDateTime,
  ...
  const handleEventClick = (info) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .patch(`http://localhost:8001/api/events/${event.id}/`, { active: false })
          .then(() => {
            event.remove();
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 font-sans">
      {/* <div className="text-xl font-bold mb-3">📅 {formatDateToMonthYear(currentDate)}</div> */}
      ...
      <FullCalendar
        ...
        events={events}
        eventDisplay="block"
        displayEventTime={false}   // listedeki "12 ..." sorunu kalkar
        height="auto"
        nowIndicator={true}
        editable={true}
        selectable={true}
