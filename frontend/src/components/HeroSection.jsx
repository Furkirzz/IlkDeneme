...
// Metin alanları obje/array gelirse güvenle string'e çevir
const toText = (v) => {
  if (v == null) return "";
  if (typeof v === "string" || typeof v === "number") return String(v);
  if (Array.isArray(v)) return v.map(toText).filter(Boolean).join(", ");
  if (typeof v === "object") {
    for (const k of ["title", "name", "label", "value", "text"]) {
      if (v[k] != null) return toText(v[k]);
    }
    return "";
  }
  return "";
...
const HeroSection = () => {
  ...
  useEffect(() => {
    const fetchHeroImages = async () => {
      try {
        // debug: hangi baseURL'e gidiyoruz?
        ...
        const formatted = filtered.map((img) => {
          ...
          return {
            ...img,
            fullImageUrl: url,
            _titleText: toText(img.title),
            _descText: toText(img.description),
          };
        });
  ...
  const cur = heroImages[currentImageIndex];
                ...
                {imageLoading ? (
                  ...
                  <div className="relative group">
                    <div className="relative overflow-hidden rounded-2xl">
                      <img
                        src={cur.fullImageUrl}
                        alt={toText(cur._titleText) || "Hero Image"}
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4 rounded-b-2xl">
                        {toText(cur._titleText) && (
                          <h3 className="text-white font-semibold text-lg mb-1">
                            {toText(cur._titleText)}
                          </h3>
                        )}
                        {toText(cur._descText) && (
