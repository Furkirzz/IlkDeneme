...
function UploadResult() {
  ...
  // Sadece A–F, boşluk yok; uppercase; max 90
  const sanitizeAnswer = (val) => val.toUpperCase().replace(/[^A-F]/g, "").slice(0, 90);
  ...
  const isValidAnswers = cevapAnahtariA.length === 90 && cevapAnahtariB.length === 90;

  const canSubmit =
    !loading && denemeAdi.trim().length >= 3 && !!file && isValidAnswers;
  ...
  const handleUpload = async () => {
    ...
    if (!/\.txt$/i.test(file.name)) {
      setMessage("Sadece .txt dosyaları kabul edilir.");
      return;
    }
    if (!isValidAnswers) {
      setMessage("Cevap anahtarları A–F harflerinden oluşmalı ve tam 90 karakter olmalıdır.");
      return;
    }
    ...
    try {
      setLoading(true);
      // NOT: baseURL ve Authorization header'ı api içinde ayarlı
      const res = await api.post("/upload-results/", formData);
      ...
      const hataMesaji =
        ...
        (err?.response?.status === 401
          ? "Yetkisiz: Lütfen giriş yapın ve tekrar deneyin."
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-bold text-gray-800 mb-4">Sınav Sonuçları Yükleme</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Deneme adını girin, öğrenci cevap dosyanızı ve A &amp; B kitapçığı cevap anahtarlarını yükleyerek sonuçları
            hızlıca analiz edin.
          </p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-10 border border-gray-100">
          ...
          <div className="mb-10">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-2xl font-semibold text-gray-800">Deneme Adı</h2>
            </div>
              ...
              <div className="flex justify-between items-center mt-3">
                ...
                <div
                  className={`px-3 py-1 rounded-full text-sm font-medium ${
                    denemeAdi.trim().length >= 3 ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"
                  }`}
                >
          ...
          <div className="mb-10">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-2xl font-semibold text-gray-800">Dosya Seçimi</h2>
            </div>
            ...
            <div className="relative">
              ...
              <label htmlFor="file-upload">
                <div
                  className={`border-3 border-dashed rounded-2xl p-8 text-center transition-all duration-300 hover:scale-[1.02] ${
                  ...
                  <div className="flex flex-col items-center">
                    {file ? (
                      <>
                        ...
                        <p className="text-xl font-semibold text-green-700 mb-2">Dosya Seçildi!</p>
                        <p className="text-green-600 mb-4">{file.name}</p>
                        <p className="text-sm text-green-500">Boyut: {(file.size / 1024).toFixed(2)} KB</p>
                      </>
                    ) : (
                      <>
                        <FiFileText className="w-16 h-16 text-gray-400 mb-4" />
                        <p className="text-xl font-semibold text-gray-700 mb-2">.txt Dosyasını Seçin</p>
                        <p className="text-gray-500 mb-4">Öğrenci cevaplarını içeren dosyayı buraya sürükleyin veya tıklayın</p>
          ...
          <div className="text-center">
            <button
              ...
              disabled={!canSubmit}
              className={`inline-flex items-center px-8 py-4 rounded-2xl text-lg font-bold transition-all duration-300 transform min-w-[200px] ${
                !canSubmit
              ...
              title={!canSubmit ? "Deneme adı / dosya / cevap anahtarlarını kontrol edin." : ""}
            >
              {loading ? (
                <>
          ...
          {message && (
            <div
              className={`mt-8 p-6 rounded-2xl border-l-4 ${
                ...
                message.toLowerCase().includes("karakter") ||
        ...
        <div className="grid md:grid-cols-4 gap-6 mt-8">
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            ...
            <p className="text-gray-600 text-sm">Her deneme için benzersiz bir ad verin. Aynı isimde deneme varsa güncellenecektir.</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            ...
            <p className="text-gray-600 text-sm">Sadece .txt formatında dosyalar kabul edilir. Dosya UTF-8 kodlamasında olmalıdır.</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            ...
            <p className="text-gray-600 text-sm">Her kitapçık için tam 90 karakter girilmelidir. Sadece A–F harflerini kullanın.</p>
          </div>
